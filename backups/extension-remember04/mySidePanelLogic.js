...
function myLoadItemForEdit(myId) {
    ...
    const myItem = myItemsDataSet.get(myNumericId);
    if (myItem) {
...
function myGetNextId() {
  ...
  const myNumericIds = myAllIds.map(id => Number(id)).filter(id => !isNaN(id));
  return myNumericIds.length ? Math.max(...myNumericIds) + 1 : 1;
}
...
function myExportCsv() {
  ...
  // Headers match the SRS data structure
  const myHeaders = ["id", "content", "longDescription", "start", "myOriginalStart", "myCorrectCount"];
  let myCsv = myHeaders.map(h => `"${h}"`).join(',') + '\n';

  myAllItems.forEach(myItem => {
    // Escape quotes and ensure fields are quoted
    const myRow = myHeaders.map(myKey => {
      let myValue = myItem[myKey] !== undefined ? myItem[myKey].toString() : '';
...
function myProcessImportData() {
    ...
    try {
        ...
        myStatus.textContent = "Error: Data could not be parsed as valid JSON. Check format.";
        return;
    }

    if (myParsedData && Array.isArray(myParsedData) && myParsedData.length > 0) {
        // Clear and replace data set
        myItemsDataSet.clear();
...
// =========================================================================
// --- SRS TESTING & SCHEDULING LOGIC (Uses localStorage) ---
// =========================================================================
...
function myPopulateSelectBox() {
  ...
  // Filter for items that are due (scheduled time is in the past)
...
function myLoadQuestion(myId) {
  if (!myId) {
    myHintInput.value = '';
    myStoredStatementInput.value = '';
    myUserTestInput.value = '';
    myPresentNumberInput.value = '';
    return;
  }
  ...
  if (myItem) {
    // The Long Description (correct answer) is stored in the hidden input for the LLM check
    myStoredStatementInput.value = myItem.longDescription || '';
    myHintInput.value = myItem.content || '';
    myUserTestInput.value = '';
    myPresentNumberInput.value = myId;
  }
}
...
function myUpdateMemory(myQuestionId, myLastScheduledReviewTimeIso, myIsCorrect) {
  ...
  if (myIsCorrect) {
    ...
    // Calculate new interval using the Ease Factor
    const myNewInterval = myDuration * myEaseFactor.value;
    const myNextDateTime = myNow + myNewInterval;

    // Adjust Ease Factor (EF) upwards slightly
    const myEFDelta = myFactorAdjustment;
    myEaseFactor.value = Math.min(myEaseFactor.value + myEFDelta, myFactorMax);
    myAdjustmentMessage = `EF ↑ by ${myEFDelta.toFixed(2)}.`;
    ...
    // Failure Logic: Reset interval and decrease EF
    const myNewInterval = 60000; // Reset interval to 1 minute
    const myNextDateTime = myNow + myNewInterval;
    myItemToUpdate.myCorrectCount = 0;

    // Adjust Ease Factor (EF) downwards more significantly
    const myEFDelta = myFactorAdjustment * 2;
...
async function myCheckSimilarity() {
  ...
  const myStoredValue = myStoredStatementInput.value.trim();
  ...
  try {
    ...
    // Recreate session if needed, using the fixed system instruction for comparison
    ...
    myStopTimer("An error occurred during AI comparison. Check the console for details.");
    console.error("Similarity Check Error:", myError);
  } finally {
    myCheckButton.disabled = false;
    myCheckButton.style.backgroundColor = '#4CAF50';
    myPopulateSelectBox(); // Update the list of due questions
  }
}
